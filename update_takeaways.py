...
publishes (energy universe movers + sectors, hot tickers, tape).
...
def main():
    ...
    for it in (data or {}).get("tape", []) or []:
        lab = (it.get("label") or "")
        if any(k in lab for k in ("SPX", "Nasdaq", "Dow", "WTI", "Brent", "NatGas")):
    ...
    if closed_reason:
        mbody = f"At the last close, {breadth}% of {tot} energy names were green — {tone}."
    else:
        mbody = f"{breadth}% of {tot} energy names are green — {tone}."
    if tape_bits:
    ...
    cards.append({
        "headline": (f"Last close: {tone}, {breadth}% of energy names green" if closed_reason
                     else f"The tape: {tone}, {breadth}% of energy names green"),
    ...
    if strong and weak and strong is not weak:
        cards.append({
            ...
            "body": (f"Strongest energy sector is {strong['label']} at {pct(strong['avg_change_pct'])}; "
                     f"weakest is {weak['label']} at {pct(weak['avg_change_pct'])}. "
                     f"Rotation within the energy trade, not a one-way move."),
            "tickers": [], "watch": None})
    ...
    # 4. hot, no-catalyst single-name card
    ...
    sl = f"{breadth}% of {tot} energy names green"
    if strong:
