# UEC Research Report
**Uranium Energy Corp.** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `UEC.research.03.08.2026`

| Field | Value |
|---|---|
| Current price | $9.97 (2026-08-03, ~15-min delayed) |
| Market cap | $4.89B |
| Sector / Industry | Energy / Uranium |
| Main theme | Uranium — see catalysts below |
| Current stance | Low technical momentum, mixed fundamentals, value: reasonable, risk: high. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-03.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Low | Below SMA200 (-26.17%) with negative half-year (-42.17%). |
| Fresh setup quality | Poor / broken | -50.50% from 52w high — base needs to rebuild. |
| Fundamental quality | Mixed | Profit margin 0.00%, revenue growth —. |
| Value attractiveness | Reasonable | Forward P/E -99.70, EV/Sales 211.12. |
| Risk level | High | Beta 1.21, ATR 6.5% of price, short float 14.14%. |

**Bottom line:** Low technical momentum, mixed fundamentals, value: reasonable, risk: high.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Uranium Energy Corp., together with its subsidiaries, engages in exploration, pre-extraction, extraction, and processing of uranium and titanium concentrates properties in the United States, Canada, and the Republic of Paraguay. The company was formerly known as Carlin Gold Inc. and changed its name to Uranium Energy Corp. in January 2005. The company was incorporated in 2003 and is headquartered in Corpus Christi, Texas.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Uranium Energy (UEC) Stock Looks Stretched Despite Its 353% Five Year Run | Simply Wall St. | 2026-07-31 |
| Uranium Energy Is Down Sharply in 2026. Here's What the Next 10 Years Could Realistically Look Like. | Motley Fool | 2026-07-30 |
| The Next AI Trade: Fireside Chat with Noble Plains Uranium CEO Drew Zimmerman | IPO-Edge.com | 2026-07-29 |
| Should Cameco Stock be in Your Portfolio Before Q2 Earnings? | Zacks | 2026-07-28 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $9.97; 52w high $20.14 (-50.50%); 52w low $8.67 (+14.99%) |
| Trend | -26.17% vs SMA200, -10.25% vs SMA50, +2.04% vs SMA20 |
| Momentum | RSI(14) 48.0 (neutral) |
| Volatility | ATR(14) 0.65 (~6.5% of price); beta 1.21 |
| Setup perspective | -50.50% from 52w high — base needs to rebuild. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +0.6% |
| Month | -7.4% |
| Quarter | -33.3% |
| Half Y | -42.2% |
| 1Y | +15.0% |
| YTD | -24.0% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-09-24 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $20.20M | Revenue growth — y/y |
| Profitability | Gross 0.00%, operating -629.66%, net 0.00% | ROA -6.25%, ROE -8.96% |
| Balance sheet | Cash $488.05M, debt $1.91M | Current ratio 32.67, debt/equity 0.14 |
| Valuation | P/E —, forward P/E -99.70, P/S 241.96, P/B 3.46 | EV/Sales 211.12, EV/EBITDA -35.31 |
| Growth expectations | Earnings growth —, EPS q/q — | Analyst mean target $18.03 (9 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 84.98% |
| Insider ownership | 1.75% |
| Short float | 14.14% |
| Short ratio (days to cover) | 6.3 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Price (T.Rowe) Associates Inc | 75,546,034 | 15.27% | 2026-03-31 |
| Blackrock Inc. | 37,507,091 | 7.58% | 2026-03-31 |
| Mirae Asset Global ETFs Holdings Ltd. | 34,102,140 | 6.89% | 2026-03-31 |
| State Street Corporation | 25,197,697 | 5.09% | 2026-03-31 |
| Van Eck Associates Corporation | 23,842,727 | 4.82% | 2026-03-31 |
| Vanguard Portfolio Management LLC | 23,330,789 | 4.71% | 2026-03-31 |
| Vanguard Capital Management LLC | 21,638,404 | 4.37% | 2026-03-31 |
| Geode Capital Management, LLC | 12,712,165 | 2.57% | 2026-03-31 |
| Alps Advisors Inc. | 9,829,329 | 1.99% | 2026-06-30 |
| Driehaus Capital Management, LLC | 8,334,245 | 1.68% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Volatility risk:** Beta 1.21, ATR 6.5% of price, short float 14.14%. Size positions accordingly.
- **Short interest risk:** short float 14.14% can fuel squeezes both ways around news.
- **Trend risk:** far below the 52-week high; falling-knife entries without a base are low-quality setups.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $9.97 | EV/Sales | 211.12 |
| Market cap | $4.89B | EV/EBITDA | -35.31 |
| Beta | 1.21 | Gross margin | 0.00% |
| RSI(14) | 48.0 | Operating margin | -629.66% |
| ATR(14) | 0.65 | Profit margin | 0.00% |
| SMA20 dist | +2.04% | ROA | -6.25% |
| SMA50 dist | -10.25% | ROE | -8.96% |
| SMA200 dist | -26.17% | Revenue (ttm) | $20.20M |
| 52W high | $20.14 | Revenue growth y/y | — |
| 52W low | $8.67 | Inst. ownership | 84.98% |
| P/E (ttm) | — | Insider ownership | 1.75% |
| Forward P/E | -99.70 | Short float | 14.14% |
| PEG (trailing) | — | Avg volume | 9,994,761 |
| P/S | 241.96 | Employees | 171 |
| P/B | 3.46 | Analyst rec (1=buy..5=sell) | 1.4 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-06-10 | reit | HC Wainwright & Co. | Buy → Buy |
| 2026-05-20 | reit | HC Wainwright & Co. | Buy → Buy |
| 2026-03-11 | main | TD Securities | Buy → Buy |
| 2026-03-11 | main | HC Wainwright & Co. | Buy → Buy |
| 2025-12-11 | reit | Roth Capital | Buy → Buy |
| 2025-09-26 | main | Goldman Sachs | Buy → Buy |
| 2025-09-25 | main | Roth Capital | Buy → Buy |
| 2025-09-25 | main | HC Wainwright & Co. | Buy → Buy |

## 9. Conclusion
UEC: Low momentum / mixed fundamentals / reasonable value / high risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for UEC | https://finance.yahoo.com/quote/UEC | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/UEC/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/UEC/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._