# NXT Research Report
**Nextpower Inc.** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `NXT.research.25.08.2026`

| Field | Value |
|---|---|
| Current price | $84.81 (2026-08-25, ~15-min delayed) |
| Market cap | $13.03B |
| Sector / Industry | Technology / Solar |
| Main theme | Solar — see catalysts below |
| Current stance | Low technical momentum, mixed fundamentals, value: reasonable, risk: high. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-25.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Low | Below SMA200 (-22.22%) with negative half-year (-29.66%). |
| Fresh setup quality | Poor / broken | -45.77% from 52w high — base needs to rebuild. |
| Fundamental quality | Mixed | Profit margin 16.36%, revenue growth 8.20%. |
| Value attractiveness | Reasonable | Forward P/E 14.68, EV/Sales 3.22. |
| Risk level | High | Beta 1.95, ATR 6.6% of price, short float 7.57%. |

**Bottom line:** Low technical momentum, mixed fundamentals, value: reasonable, risk: high.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Nextpower Inc. provides solar and energy technology solutions for utility-scale power plants in the United States and internationally. It offers tracking solutions, including NX Horizon, a solar tracking solution; NX Horizon-XTR, a terrain-following tracker to expand the addressable market for trackers on sites with sloped, uneven, and challenging terrain; NX Horizon Hail Pro adds automatic stowing using weather service information; and NX Horizon Low Carbon, a solar tracker solution with a reduced carbon footprint.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Unpacking Q2 Earnings: Nextpower (NASDAQ:NXT) In The Context Of Other Renewable Energy Stocks | StockStory | 2026-08-25 |
| 2 of Wall Street’s Favorite Stocks to Own for Decades and 1 We Ignore | StockStory | 2026-08-21 |
| Nextpower, Array, and American Superconductor Shares Plummet, What You Need To Know | StockStory | 2026-08-20 |
| Nextpower Highlights Record Revenue, $5.5B Backlog and Clean-Energy Expansion | MarketBeat | 2026-08-19 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $84.81; 52w high $156.40 (-45.77%); 52w low $64.33 (+31.84%) |
| Trend | -22.22% vs SMA200, -19.31% vs SMA50, -11.35% vs SMA20 |
| Momentum | RSI(14) 33.3 (neutral) |
| Volatility | ATR(14) 5.62 (~6.6% of price); beta 1.95 |
| Setup perspective | -45.77% from 52w high — base needs to rebuild. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | -7.0% |
| Month | -18.2% |
| Quarter | -35.4% |
| Half Y | -29.7% |
| 1Y | +23.3% |
| YTD | -8.6% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-10-28 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $3.63B | Revenue growth 8.20% y/y |
| Profitability | Gross 22.98%, operating 20.86%, net 16.36% | ROA 11.68%, ROE 27.22% |
| Balance sheet | Cash $1.21B, debt $38.40M | Current ratio 2.69, debt/equity 1.50 |
| Valuation | P/E 21.91, forward P/E 14.68, P/S 3.59, P/B 5.03 | EV/Sales 3.22, EV/EBITDA 15.59 |
| Growth expectations | Earnings growth 2.90%, EPS q/q 5.20% | Analyst mean target $141.52 (27 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 109.54% |
| Insider ownership | 0.63% |
| Short float | 7.57% |
| Short ratio (days to cover) | 2.7 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Blackrock Inc. | 21,140,588 | 13.93% | 2026-06-30 |
| FMR, LLC | 18,753,360 | 12.36% | 2026-06-30 |
| Primecap Management Company | 6,688,767 | 4.41% | 2026-06-30 |
| Vanguard Capital Management LLC | 6,504,906 | 4.29% | 2026-06-30 |
| Vanguard Portfolio Management LLC | 5,956,173 | 3.93% | 2026-06-30 |
| Invesco Ltd. | 4,868,539 | 3.21% | 2026-06-30 |
| State Street Corporation | 4,741,459 | 3.12% | 2026-06-30 |
| Geode Capital Management, LLC | 3,423,197 | 2.26% | 2026-06-30 |
| Wellington Management Group, LLP | 3,427,333 | 2.26% | 2026-06-30 |
| Fundsmith LLP | 2,856,092 | 1.88% | 2026-06-30 |

## 6. Risk Review and Setup Plan
- **Volatility risk:** Beta 1.95, ATR 6.6% of price, short float 7.57%. Size positions accordingly.
- **Short interest risk:** short float 7.57% can fuel squeezes both ways around news.
- **Trend risk:** far below the 52-week high; falling-knife entries without a base are low-quality setups.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $84.81 | EV/Sales | 3.22 |
| Market cap | $13.03B | EV/EBITDA | 15.59 |
| Beta | 1.95 | Gross margin | 22.98% |
| RSI(14) | 33.3 | Operating margin | 20.86% |
| ATR(14) | 5.62 | Profit margin | 16.36% |
| SMA20 dist | -11.35% | ROA | 11.68% |
| SMA50 dist | -19.31% | ROE | 27.22% |
| SMA200 dist | -22.22% | Revenue (ttm) | $3.63B |
| 52W high | $156.40 | Revenue growth y/y | 8.20% |
| 52W low | $64.33 | Inst. ownership | 109.54% |
| P/E (ttm) | 21.91 | Insider ownership | 0.63% |
| Forward P/E | 14.68 | Short float | 7.57% |
| PEG (trailing) | 3.15 | Avg volume | 3,056,006 |
| P/S | 3.59 | Employees | 1,993 |
| P/B | 5.03 | Analyst rec (1=buy..5=sell) | 1.5 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-08-05 | reit | GLJ Research | Buy → Buy |
| 2026-08-04 | main | Mizuho | Neutral → Neutral |
| 2026-08-03 | main | Citigroup | Buy → Buy |
| 2026-07-31 | main | RBC Capital | Outperform → Outperform |
| 2026-07-31 | main | JP Morgan | Overweight → Overweight |
| 2026-07-31 | main | Truist Securities | Buy → Buy |
| 2026-07-31 | main | Susquehanna | Positive → Positive |
| 2026-07-28 | reit | Roth Capital | Buy → Buy |

## 9. Conclusion
NXT: Low momentum / mixed fundamentals / reasonable value / high risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for NXT | https://finance.yahoo.com/quote/NXT | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/NXT/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/NXT/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._