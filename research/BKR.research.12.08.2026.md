# BKR Research Report
**Baker Hughes Company** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `BKR.research.12.08.2026`

| Field | Value |
|---|---|
| Current price | $64.28 (2026-08-12, ~15-min delayed) |
| Market cap | $63.81B |
| Sector / Industry | Energy / Oil & Gas Equipment & Services |
| Main theme | Oil & Gas Equipment & Services — see catalysts below |
| Current stance | Moderate technical momentum, mixed fundamentals, value: fair-to-demanding, risk: moderate. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-12.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Moderate | 1Y +52.89%; price +13.21% vs SMA200. |
| Fresh setup quality | Moderate / wait | -7.09% from 52w high; no clean fresh pivot by default. |
| Fundamental quality | Mixed | Profit margin 11.17%, revenue growth -2.40%. |
| Value attractiveness | Fair-to-demanding | Forward P/E 20.31, EV/Sales 2.30. |
| Risk level | Moderate | Beta 0.96, ATR 2.7% of price, short float 2.83%. |

**Bottom line:** Moderate technical momentum, mixed fundamentals, value: fair-to-demanding, risk: moderate.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Baker Hughes Company provides a portfolio of technologies and services to energy and industrial value chain. Its Oilfield Services & Equipment segment designs and manufactures exploration, appraisal, development, production, rejuvenation, and decommissioning products and related services for onshore and offshore oilfield operations.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| 5 LNG Megaprojects Poised to Power the Next Gas Boom | Oilprice.com | 2026-08-12 |
| Baker Hughes wins KOC contract for Ahmadi Innovation Valley project | Offshore Technology | 2026-08-12 |
| Sector Update: Energy Stocks Gain Late Afternoon | MT Newswires | 2026-08-11 |
| Sector Update: Energy Stocks Gain Tuesday Afternoon | MT Newswires | 2026-08-11 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $64.28; 52w high $69.18 (-7.09%); 52w low $41.86 (+53.58%) |
| Trend | +13.21% vs SMA200, +8.56% vs SMA50, +8.00% vs SMA20 |
| Momentum | RSI(14) 66.3 (neutral) |
| Volatility | ATR(14) 1.76 (~2.7% of price); beta 0.96 |
| Setup perspective | -7.09% from 52w high; no clean fresh pivot by default. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +4.6% |
| Month | +11.7% |
| Quarter | -1.1% |
| Half Y | +9.9% |
| 1Y | +52.9% |
| YTD | +37.8% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-10-22 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $27.73B | Revenue growth -2.40% y/y |
| Profitability | Gross 23.66%, operating 12.83%, net 11.17% | ROA 4.85%, ROE 16.46% |
| Balance sheet | Cash $17.02B, debt $16.25B | Current ratio 2.10, debt/equity 80.92 |
| Valuation | P/E 20.67, forward P/E 20.31, P/S 2.30, P/B 3.20 | EV/Sales 2.30, EV/EBITDA 13.17 |
| Growth expectations | Earnings growth -4.20%, EPS q/q -2.90% | Analyst mean target $71.39 (23 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 100.15% |
| Insider ownership | 0.15% |
| Short float | 2.83% |
| Short ratio (days to cover) | 2.4 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Blackrock Inc. | 99,116,387 | 11.96% | 2026-06-30 |
| State Street Corporation | 64,578,131 | 7.79% | 2026-06-30 |
| Vanguard Capital Management LLC | 64,226,792 | 7.75% | 2026-03-31 |
| JPMORGAN CHASE & CO | 58,196,567 | 7.02% | 2026-03-31 |
| Vanguard Portfolio Management LLC | 48,783,931 | 5.89% | 2026-03-31 |
| Capital World Investors | 48,433,252 | 5.85% | 2026-03-31 |
| Dodge & Cox Inc. | 35,871,558 | 4.33% | 2026-03-31 |
| Invesco Ltd. | 35,424,793 | 4.28% | 2026-03-31 |
| Geode Capital Management, LLC | 26,847,011 | 3.24% | 2026-03-31 |
| Capital Research Global Investors | 21,963,612 | 2.65% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Valuation risk:** Forward P/E 20.31, EV/Sales 2.30. Multiple compression is the main downside if growth disappoints.
- **Technical trap:** near 52-week highs — do not confuse an old breakout that already worked with a fresh entry.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $64.28 | EV/Sales | 2.30 |
| Market cap | $63.81B | EV/EBITDA | 13.17 |
| Beta | 0.96 | Gross margin | 23.66% |
| RSI(14) | 66.3 | Operating margin | 12.83% |
| ATR(14) | 1.76 | Profit margin | 11.17% |
| SMA20 dist | +8.00% | ROA | 4.85% |
| SMA50 dist | +8.56% | ROE | 16.46% |
| SMA200 dist | +13.21% | Revenue (ttm) | $27.73B |
| 52W high | $69.18 | Revenue growth y/y | -2.40% |
| 52W low | $41.86 | Inst. ownership | 100.15% |
| P/E (ttm) | 20.67 | Insider ownership | 0.15% |
| Forward P/E | 20.31 | Short float | 2.83% |
| PEG (trailing) | 2.20 | Avg volume | 8,654,279 |
| P/S | 2.30 | Employees | 54,000 |
| P/B | 3.20 | Analyst rec (1=buy..5=sell) | 1.7 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-07-28 | main | UBS | Neutral → Neutral |
| 2026-07-28 | main | TD Cowen | Buy → Buy |
| 2026-07-28 | main | Piper Sandler | Overweight → Overweight |
| 2026-07-28 | main | Stifel | Buy → Buy |
| 2026-07-28 | main | Susquehanna | Positive → Positive |
| 2026-07-16 | main | Barclays | Equal-Weight → Equal-Weight |
| 2026-07-14 | main | Piper Sandler | Overweight → Overweight |
| 2026-07-08 | main | Citigroup | Buy → Buy |

## 9. Conclusion
BKR: Moderate momentum / mixed fundamentals / fair-to-demanding value / moderate risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for BKR | https://finance.yahoo.com/quote/BKR | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/BKR/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/BKR/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._