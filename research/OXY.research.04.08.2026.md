# OXY Research Report
**Occidental Petroleum Corporation** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `OXY.research.04.08.2026`

| Field | Value |
|---|---|
| Current price | $55.09 (2026-08-04, ~15-min delayed) |
| Market cap | $54.79B |
| Sector / Industry | Energy / Oil & Gas E&P |
| Main theme | Oil & Gas E&P — see catalysts below |
| Current stance | Moderate technical momentum, mixed fundamentals, value: reasonable, risk: elevated. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-04.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Moderate | 1Y +29.53%; price +10.10% vs SMA200. |
| Fresh setup quality | Moderate / wait | -16.45% from 52w high; no clean fresh pivot by default. |
| Fundamental quality | Mixed | Profit margin 22.42%, revenue growth -8.30%. |
| Value attractiveness | Reasonable | Forward P/E 14.09, EV/Sales 3.64. |
| Risk level | Elevated | Beta 0.16, ATR 3.1% of price, short float 0.01%. |

**Bottom line:** Moderate technical momentum, mixed fundamentals, value: reasonable, risk: elevated.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Occidental Petroleum Corporation, together with its subsidiaries, engages in the acquisition, exploration, and development of oil and gas properties in the United States and internationally. It operates through Oil and Gas and Midstream and Marketing. The Oil and Gas segment explores for, develops, and produces oil and condensate, natural gas liquids (NGLs), and natural gas. This segment also optimizes its transportation and storage capacity and invests in entities.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Countdown to Occidental (OXY) Q2 Earnings: Wall Street Forecasts for Key Metrics | Zacks | 2026-08-04 |
| OXY Stock: When A Bargain Price Is A Message | Trefis | 2026-08-04 |
| ConocoPhillips Gears Up to Report Q2 Earnings: What's in the Cards? | Zacks | 2026-08-03 |
| Plug Power vs. Occidental Petroleum: Which Energy Stock Is a Better Buy in 2026? | Motley Fool | 2026-08-02 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $55.09; 52w high $65.94 (-16.45%); 52w low $38.56 (+42.88%) |
| Trend | +10.10% vs SMA200, +0.98% vs SMA50, -0.11% vs SMA20 |
| Momentum | RSI(14) 50.9 (neutral) |
| Volatility | ATR(14) 1.68 (~3.1% of price); beta 0.16 |
| Setup perspective | -16.45% from 52w high; no clean fresh pivot by default. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +2.2% |
| Month | +12.9% |
| Quarter | -8.2% |
| Half Y | +27.0% |
| 1Y | +29.5% |
| YTD | +31.2% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-08-05 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $21.12B | Revenue growth -8.30% y/y |
| Profitability | Gross 69.76%, operating 17.72%, net 22.42% | ROA 2.50%, ROE 4.05% |
| Balance sheet | Cash $3.81B, debt $16.61B | Current ratio 1.21, debt/equity 41.99 |
| Valuation | P/E 74.45, forward P/E 14.09, P/S 2.59, P/B 1.78 | EV/Sales 3.64, EV/EBITDA 7.10 |
| Growth expectations | Earnings growth 315.60%, EPS q/q 257.40% | Analyst mean target $64.52 (23 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 55.24% |
| Insider ownership | 26.94% |
| Short float | 0.01% |
| Short ratio (days to cover) | 0.0 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Berkshire Hathaway, Inc | 264,941,431 | 26.64% | 2026-03-31 |
| Dodge & Cox Inc. | 74,085,572 | 7.45% | 2026-03-31 |
| Blackrock Inc. | 52,637,811 | 5.29% | 2026-03-31 |
| Vanguard Capital Management LLC | 47,412,976 | 4.77% | 2026-03-31 |
| State Street Corporation | 43,270,797 | 4.35% | 2026-03-31 |
| Vanguard Portfolio Management LLC | 36,185,199 | 3.64% | 2026-03-31 |
| Geode Capital Management, LLC | 19,368,894 | 1.95% | 2026-03-31 |
| GQG Partners LLC | 18,311,639 | 1.84% | 2026-03-31 |
| Morgan Stanley | 10,976,687 | 1.10% | 2026-03-31 |
| H&H International Investment, LLC | 10,261,500 | 1.03% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Volatility risk:** Beta 0.16, ATR 3.1% of price, short float 0.01%. Size positions accordingly.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $55.09 | EV/Sales | 3.64 |
| Market cap | $54.79B | EV/EBITDA | 7.10 |
| Beta | 0.16 | Gross margin | 69.76% |
| RSI(14) | 50.9 | Operating margin | 17.72% |
| ATR(14) | 1.68 | Profit margin | 22.42% |
| SMA20 dist | -0.11% | ROA | 2.50% |
| SMA50 dist | +0.98% | ROE | 4.05% |
| SMA200 dist | +10.10% | Revenue (ttm) | $21.12B |
| 52W high | $65.94 | Revenue growth y/y | -8.30% |
| 52W low | $38.56 | Inst. ownership | 55.24% |
| P/E (ttm) | 74.45 | Insider ownership | 26.94% |
| Forward P/E | 14.09 | Short float | 0.01% |
| PEG (trailing) | 1.20 | Avg volume | 10,337,088 |
| P/S | 2.59 | Employees | 10,412 |
| P/B | 1.78 | Analyst rec (1=buy..5=sell) | 2.3 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-07-17 | main | Citigroup | Neutral → Neutral |
| 2026-07-14 | main | Stephens & Co. | Overweight → Overweight |
| 2026-07-08 | up | Evercore ISI Group | Underperform → Outperform |
| 2026-06-29 | main | Morgan Stanley | Equal-Weight → Equal-Weight |
| 2026-05-27 | main | Mizuho | Outperform → Outperform |
| 2026-05-26 | up | Barclays | Equal-Weight → Overweight |
| 2026-05-22 | main | Morgan Stanley | Equal-Weight → Equal-Weight |
| 2026-05-08 | main | Truist Securities | Hold → Hold |

## 9. Conclusion
OXY: Moderate momentum / mixed fundamentals / reasonable value / elevated risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for OXY | https://finance.yahoo.com/quote/OXY | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/OXY/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/OXY/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._