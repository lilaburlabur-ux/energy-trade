# OKLO Research Report
**Oklo Inc.** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `OKLO.research.12.08.2026`

| Field | Value |
|---|---|
| Current price | $45.13 (2026-08-12, ~15-min delayed) |
| Market cap | $8.39B |
| Sector / Industry | Utilities / Utilities - Independent Power Producers |
| Main theme | Utilities - Independent Power Producers — see catalysts below |
| Current stance | Low technical momentum, mixed fundamentals, value: reasonable, risk: high. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-12.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Low | Below SMA200 (-36.79%) with negative half-year (-35.04%). |
| Fresh setup quality | Poor / broken | -74.08% from 52w high — base needs to rebuild. |
| Fundamental quality | Mixed | Profit margin 0.00%, revenue growth —. |
| Value attractiveness | Reasonable | Forward P/E -44.94, EV/Sales 5,192.97. |
| Risk level | High | Beta 1.20, ATR 8.1% of price, short float 18.60%. |

**Bottom line:** Low technical momentum, mixed fundamentals, value: reasonable, risk: high.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Oklo Inc. develops fission power plants to provide energy at scale to customers in the United States. The company offers Aurora Powerhouse, which is designed to produce between 15 and up to 75 megawatts of electricity. It is also commercializing nuclear fuel recycling and fuel fabrication technology that can convert used nuclear fuel into usable fuel for its reactors. The company was formerly known as AltC Acquisition Corp. and changed its name to Oklo Inc. in May 2024. Oklo Inc. was founded in 2013 and is headquartered in Santa Clara, California.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Can NuScale's TVA Opportunity Unlock Its Next Growth Phase? | Zacks | 2026-08-12 |
| Can NuScale's Commercial Potential Justify Its Valuation? | Zacks | 2026-08-12 |
| Should You Buy, Sell or Hold OKLO Stock Post First Revenues? | Zacks | 2026-08-12 |
| Why Oklo (OKLO) Is Up 8.5% After First Criticality And Initial Reactor Sales - And What's Next | Simply Wall St. | 2026-08-12 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $45.13; 52w high $174.14 (-74.08%); 52w low $36.84 (+22.50%) |
| Trend | -36.79% vs SMA200, -9.69% vs SMA50, +6.17% vs SMA20 |
| Momentum | RSI(14) 49.6 (neutral) |
| Volatility | ATR(14) 3.67 (~8.1% of price); beta 1.20 |
| Setup perspective | -74.08% from 52w high — base needs to rebuild. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +5.0% |
| Month | -2.4% |
| Quarter | -38.7% |
| Half Y | -35.0% |
| 1Y | -37.2% |
| YTD | -42.0% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-08-07 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $1.21M | Revenue growth — y/y |
| Profitability | Gross 40.41%, operating -6048.76%, net 0.00% | ROA -6.65%, ROE -7.70% |
| Balance sheet | Cash $2.47B, debt $3.96M | Current ratio 48.46, debt/equity 0.12 |
| Valuation | P/E —, forward P/E -44.94, P/S 6,938.00, P/B 2.55 | EV/Sales 5,192.97, EV/EBITDA -28.98 |
| Growth expectations | Earnings growth —, EPS q/q — | Analyst mean target $79.88 (20 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 46.58% |
| Insider ownership | 15.42% |
| Short float | 18.60% |
| Short ratio (days to cover) | 3.5 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Mirae Asset Global ETFs Holdings Ltd. | 8,856,496 | 4.76% | 2026-03-31 |
| Blackrock Inc. | 7,311,968 | 3.93% | 2026-06-30 |
| Vanguard Portfolio Management LLC | 6,237,918 | 3.35% | 2026-03-31 |
| Van Eck Associates Corporation | 6,081,517 | 3.27% | 2026-03-31 |
| Vanguard Capital Management LLC | 5,830,933 | 3.13% | 2026-03-31 |
| Defiance Etfs, LLC | 3,643,044 | 1.96% | 2026-03-31 |
| Geode Capital Management, LLC | 3,054,149 | 1.64% | 2026-03-31 |
| Morgan Stanley | 2,681,738 | 1.44% | 2026-03-31 |
| State Street Corporation | 2,378,058 | 1.28% | 2026-06-30 |
| Alyeska Investment Group, L.p. | 1,709,692 | 0.92% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Volatility risk:** Beta 1.20, ATR 8.1% of price, short float 18.60%. Size positions accordingly.
- **Short interest risk:** short float 18.60% can fuel squeezes both ways around news.
- **Trend risk:** far below the 52-week high; falling-knife entries without a base are low-quality setups.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $45.13 | EV/Sales | 5,192.97 |
| Market cap | $8.39B | EV/EBITDA | -28.98 |
| Beta | 1.20 | Gross margin | 40.41% |
| RSI(14) | 49.6 | Operating margin | -6048.76% |
| ATR(14) | 3.67 | Profit margin | 0.00% |
| SMA20 dist | +6.17% | ROA | -6.65% |
| SMA50 dist | -9.69% | ROE | -7.70% |
| SMA200 dist | -36.79% | Revenue (ttm) | $1.21M |
| 52W high | $174.14 | Revenue growth y/y | — |
| 52W low | $36.84 | Inst. ownership | 46.58% |
| P/E (ttm) | — | Insider ownership | 15.42% |
| Forward P/E | -44.94 | Short float | 18.60% |
| PEG (trailing) | — | Avg volume | 11,467,414 |
| P/S | 6,938.00 | Employees | 215 |
| P/B | 2.55 | Analyst rec (1=buy..5=sell) | 2.0 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-08-10 | main | Truist Securities | Hold → Hold |
| 2026-08-10 | main | Citigroup | Neutral → Neutral |
| 2026-08-10 | reit | HC Wainwright & Co. | Buy → Buy |
| 2026-08-10 | main | Canaccord Genuity | Buy → Buy |
| 2026-07-23 | main | Barclays | Overweight → Overweight |
| 2026-07-14 | init | Truist Securities | — → Hold |
| 2026-06-25 | init | Guggenheim | — → Neutral |
| 2026-06-11 | main | UBS | Neutral → Neutral |

## 9. Conclusion
OKLO: Low momentum / mixed fundamentals / reasonable value / high risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for OKLO | https://finance.yahoo.com/quote/OKLO | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/OKLO/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/OKLO/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._