# SMR Research Report
**NuScale Power Corporation** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `SMR.research.05.08.2026`

| Field | Value |
|---|---|
| Current price | $9.38 (2026-08-05, ~15-min delayed) |
| Market cap | $3.25B |
| Sector / Industry | Industrials / Specialty Industrial Machinery |
| Main theme | Specialty Industrial Machinery — see catalysts below |
| Current stance | Low technical momentum, mixed fundamentals, value: reasonable, risk: high. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-05.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Low | Below SMA200 (-40.79%) with negative half-year (-47.54%). |
| Fresh setup quality | Poor / broken | -82.44% from 52w high — base needs to rebuild. |
| Fundamental quality | Mixed | Profit margin 0.00%, revenue growth -95.80%. |
| Value attractiveness | Reasonable | Forward P/E -13.09, EV/Sales 116.61. |
| Risk level | High | Beta 2.31, ATR 7.8% of price, short float 23.15%. |

**Bottom line:** Low technical momentum, mixed fundamentals, value: reasonable, risk: high.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
NuScale Power Corporation provides small modular reactor technology solutions. It offers NuScale Power Module (NPM), a light water nuclear reactor that can generate 77 megawatts of electricity (MWe). The company also provides design and nuclear regulatory licensing basis information to construct and operate the power plant; and licensing, design, development, construction, operation, and maintenance of the power plant.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Should You Invest in OKLO Stock Before Q2 Earnings Release? | Zacks | 2026-08-05 |
| Better Small Modular Reactor Stock: NuScale Power vs. GE Vernova | Motley Fool | 2026-08-03 |
| Here's How to Play NuScale Stock Before Q2 Earnings Release | Zacks | 2026-08-03 |
| NuScale Power's Next Earnings Report on Aug. 5 Could Send the Stock Soaring. Here's Why. | Motley Fool | 2026-08-02 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $9.38; 52w high $53.43 (-82.44%); 52w low $7.59 (+23.58%) |
| Trend | -40.79% vs SMA200, -4.89% vs SMA50, +10.20% vs SMA20 |
| Momentum | RSI(14) 53.5 (neutral) |
| Volatility | ATR(14) 0.73 (~7.8% of price); beta 2.31 |
| Setup perspective | -82.44% from 52w high — base needs to rebuild. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +23.6% |
| Month | +4.7% |
| Quarter | -21.0% |
| Half Y | -47.5% |
| 1Y | -78.4% |
| YTD | -42.5% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-08-05 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $18.67M | Revenue growth -95.80% y/y |
| Profitability | Gross 23.84%, operating -10181.24%, net 0.00% | ROA -14.46%, ROE -83.05% |
| Balance sheet | Cash $890.13M, debt $5.68M | Current ratio 29.36, debt/equity 0.51 |
| Valuation | P/E —, forward P/E -13.09, P/S 173.90, P/B 2.56 | EV/Sales 116.61, EV/EBITDA -10.71 |
| Growth expectations | Earnings growth —, EPS q/q — | Analyst mean target $14.57 (15 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 52.55% |
| Insider ownership | 1.99% |
| Short float | 23.15% |
| Short ratio (days to cover) | 2.3 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Van Eck Associates Corporation | 20,032,960 | 5.79% | 2026-03-31 |
| Blackrock Inc. | 12,849,132 | 3.71% | 2026-03-31 |
| Mirae Asset Global ETFs Holdings Ltd. | 11,452,161 | 3.31% | 2026-03-31 |
| Vanguard Portfolio Management LLC | 10,948,169 | 3.16% | 2026-03-31 |
| Vanguard Capital Management LLC | 10,732,371 | 3.10% | 2026-03-31 |
| Marex Group Plc | 9,098,178 | 2.63% | 2026-03-31 |
| Morgan Stanley | 7,052,969 | 2.04% | 2026-03-31 |
| Renaissance Technologies, LLC | 5,455,800 | 1.58% | 2026-03-31 |
| Marshall Wace LLP | 4,307,637 | 1.24% | 2026-03-31 |
| Geode Capital Management, LLC | 3,728,930 | 1.08% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Volatility risk:** Beta 2.31, ATR 7.8% of price, short float 23.15%. Size positions accordingly.
- **Short interest risk:** short float 23.15% can fuel squeezes both ways around news.
- **Trend risk:** far below the 52-week high; falling-knife entries without a base are low-quality setups.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $9.38 | EV/Sales | 116.61 |
| Market cap | $3.25B | EV/EBITDA | -10.71 |
| Beta | 2.31 | Gross margin | 23.84% |
| RSI(14) | 53.5 | Operating margin | -10181.24% |
| ATR(14) | 0.73 | Profit margin | 0.00% |
| SMA20 dist | +10.20% | ROA | -14.46% |
| SMA50 dist | -4.89% | ROE | -83.05% |
| SMA200 dist | -40.79% | Revenue (ttm) | $18.67M |
| 52W high | $53.43 | Revenue growth y/y | -95.80% |
| 52W low | $7.59 | Inst. ownership | 52.55% |
| P/E (ttm) | — | Insider ownership | 1.99% |
| Forward P/E | -13.09 | Short float | 23.15% |
| PEG (trailing) | — | Avg volume | 33,475,700 |
| P/S | 173.90 | Employees | 428 |
| P/B | 2.56 | Analyst rec (1=buy..5=sell) | 2.6 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-07-23 | main | Barclays | Equal-Weight → Equal-Weight |
| 2026-07-14 | init | Truist Securities | — → Hold |
| 2026-05-11 | main | Citigroup | Sell → Sell |
| 2026-04-24 | main | B. Riley Securities | Buy → Buy |
| 2026-04-23 | init | HSBC | — → Hold |
| 2026-04-21 | main | Citigroup | Sell → Sell |
| 2026-03-18 | main | UBS | Neutral → Neutral |
| 2026-03-02 | main | Citigroup | Sell → Sell |

## 9. Conclusion
SMR: Low momentum / mixed fundamentals / reasonable value / high risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for SMR | https://finance.yahoo.com/quote/SMR | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/SMR/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/SMR/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._