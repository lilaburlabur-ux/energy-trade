# PWR Research Report
**Quanta Services, Inc.** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `PWR.research.18.09.2026`

| Field | Value |
|---|---|
| Current price | $636.71 (2026-09-18, ~15-min delayed) |
| Market cap | $95.73B |
| Sector / Industry | Industrials / Engineering & Construction |
| Main theme | Engineering & Construction — see catalysts below |
| Current stance | High technical momentum, mixed fundamentals, value: fair-to-demanding, risk: elevated. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-09-18.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | High | 1Y +69.48%; price +6.85% vs SMA200. |
| Fresh setup quality | Moderate / wait | -18.90% from 52w high; no clean fresh pivot by default. |
| Fundamental quality | Mixed | Profit margin 4.03%, revenue growth 41.10%. |
| Value attractiveness | Fair-to-demanding | Forward P/E 32.30, EV/Sales 3.00. |
| Risk level | Elevated | Beta 1.20, ATR 3.8% of price, short float 2.38%. |

**Bottom line:** High technical momentum, mixed fundamentals, value: fair-to-demanding, risk: elevated.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Quanta Services, Inc. offers infrastructure solutions for the electric and gas utility, power generation, load center, manufacturing, communications, pipeline, and energy industries.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| KB Home to Report Q3 Earnings: Here's What Investors Must Know | Zacks | 2026-09-18 |
| Can Quanta's Acquisition Spree Unlock Another Growth Gear in 2026? | Zacks | 2026-09-18 |
| Buy AI-Led EME, PWR and CAT for Long-Term to Gain From Recent Softness | Zacks | 2026-09-18 |
| The Anthropic IPO Is Coming Soon. These 3 Industrial Stocks Will Profit Whether You Can Get Shares or Not. | Motley Fool | 2026-09-18 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $636.71; 52w high $785.12 (-18.90%); 52w low $388.25 (+64.00%) |
| Trend | +6.85% vs SMA200, -0.84% vs SMA50, +2.55% vs SMA20 |
| Momentum | RSI(14) 51.0 (neutral) |
| Volatility | ATR(14) 24.46 (~3.8% of price); beta 1.20 |
| Setup perspective | -18.90% from 52w high; no clean fresh pivot by default. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | -2.1% |
| Month | -6.0% |
| Quarter | -9.3% |
| Half Y | +10.2% |
| 1Y | +69.5% |
| YTD | +44.9% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-10-29 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $32.91B | Revenue growth 41.10% y/y |
| Profitability | Gross 15.46%, operating 7.22%, net 4.03% | ROA 5.23%, ROE 15.34% |
| Balance sheet | Cash $506.43M, debt $6.60B | Current ratio 1.10, debt/equity 67.78 |
| Valuation | P/E 72.77, forward P/E 32.30, P/S 2.91, P/B 9.93 | EV/Sales 3.00, EV/EBITDA 32.48 |
| Growth expectations | Earnings growth 94.70%, EPS q/q 96.90% | Analyst mean target $768.70 (29 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 94.10% |
| Insider ownership | 0.67% |
| Short float | 2.38% |
| Short ratio (days to cover) | 2.8 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Blackrock Inc. | 13,061,264 | 8.69% | 2026-06-30 |
| Vanguard Capital Management LLC | 9,802,251 | 6.52% | 2026-06-30 |
| Vanguard Portfolio Management LLC | 6,927,511 | 4.61% | 2026-06-30 |
| State Street Corporation | 6,581,807 | 4.38% | 2026-06-30 |
| JPMORGAN CHASE & CO | 5,308,495 | 3.53% | 2026-06-30 |
| Peconic Partners, LLC | 4,720,847 | 3.14% | 2026-06-30 |
| Geode Capital Management, LLC | 3,523,903 | 2.34% | 2026-06-30 |
| FMR, LLC | 3,510,968 | 2.34% | 2026-06-30 |
| Bank of America Corporation | 3,172,683 | 2.11% | 2026-06-30 |
| Morgan Stanley | 2,311,374 | 1.54% | 2026-06-30 |

## 6. Risk Review and Setup Plan
- **Valuation risk:** Forward P/E 32.30, EV/Sales 3.00. Multiple compression is the main downside if growth disappoints.
- **Volatility risk:** Beta 1.20, ATR 3.8% of price, short float 2.38%. Size positions accordingly.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $636.71 | EV/Sales | 3.00 |
| Market cap | $95.73B | EV/EBITDA | 32.48 |
| Beta | 1.20 | Gross margin | 15.46% |
| RSI(14) | 51.0 | Operating margin | 7.22% |
| ATR(14) | 24.46 | Profit margin | 4.03% |
| SMA20 dist | +2.55% | ROA | 5.23% |
| SMA50 dist | -0.84% | ROE | 15.34% |
| SMA200 dist | +6.85% | Revenue (ttm) | $32.91B |
| 52W high | $785.12 | Revenue growth y/y | 41.10% |
| 52W low | $388.25 | Inst. ownership | 94.10% |
| P/E (ttm) | 72.77 | Insider ownership | 0.67% |
| Forward P/E | 32.30 | Short float | 2.38% |
| PEG (trailing) | 1.40 | Avg volume | 1,165,214 |
| P/S | 2.91 | Employees | 69,500 |
| P/B | 9.93 | Analyst rec (1=buy..5=sell) | 1.5 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-09-09 | init | Piper Sandler | — → Overweight |
| 2026-08-07 | up | Keybanc | Sector Weight → Overweight |
| 2026-08-03 | main | TD Cowen | Buy → Buy |
| 2026-08-03 | reit | Truist Securities | Buy → Buy |
| 2026-08-03 | main | Mizuho | Neutral → Neutral |
| 2026-07-31 | main | Citigroup | Buy → Buy |
| 2026-07-21 | down | JP Morgan | Overweight → Neutral |
| 2026-07-10 | main | Jefferies | Buy → Buy |

## 9. Conclusion
PWR: High momentum / mixed fundamentals / fair-to-demanding value / elevated risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for PWR | https://finance.yahoo.com/quote/PWR | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/PWR/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/PWR/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._