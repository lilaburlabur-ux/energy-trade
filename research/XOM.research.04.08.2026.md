# XOM Research Report
**ExxonMobil** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `XOM.research.04.08.2026`

| Field | Value |
|---|---|
| Current price | $153.96 (2026-08-04, ~15-min delayed) |
| Market cap | — |
| Sector / Industry | Energy / Oil & Gas Integrated |
| Main theme | Oil & Gas Integrated — see catalysts below |
| Current stance | Moderate technical momentum, mixed fundamentals, value: reasonable, risk: moderate. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-04.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Moderate | 1Y +44.90%; price +11.18% vs SMA200. |
| Fresh setup quality | Moderate / wait | -9.60% from 52w high; no clean fresh pivot by default. |
| Fundamental quality | Mixed | Profit margin 9.07%, revenue growth 44.10%. |
| Value attractiveness | Reasonable | Forward P/E 14.58, EV/Sales 1.87. |
| Risk level | Moderate | Beta 0.17, ATR 2.5% of price, short float 1.15%. |

**Bottom line:** Moderate technical momentum, mixed fundamentals, value: reasonable, risk: moderate.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
ExxonMobil Holdings Corporation engages in the exploration and production of crude oil and natural gas in the United States, Canada, and internationally. The company operates through Upstream, Energy Products, Chemical Products, and Specialty Products segments. Its Upstream segment explores for and produces crude oil and natural gas. The Energy Products segment offers fuels, aromatics, and catalysts, as well as licensing services. Its Chemical Products segment manufactures and sells olefins, polyolefins, and intermediates.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| BP profit more than doubles as Middle East conflict boosts oil prices | Euronews | 2026-08-04 |
| Exxon Posts Its Best Profit in Four Years, Here’s Where It’ll End The Year | 24/7 Wall St. | 2026-08-04 |
| Oil Extends Losses After US, Qatar Signal Progress on Iran Draft Deal | Oilprice.com | 2026-08-04 |
| Should You Buy ExxonMobil Stock Despite Its Q2 Earnings Miss? | Zacks | 2026-08-04 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $153.96; 52w high $170.31 (-9.60%); 52w low $102.56 (+50.12%) |
| Trend | +11.18% vs SMA200, +5.20% vs SMA50, +2.67% vs SMA20 |
| Momentum | RSI(14) 59.3 (neutral) |
| Volatility | ATR(14) 3.86 (~2.5% of price); beta 0.17 |
| Setup perspective | -9.60% from 52w high; no clean fresh pivot by default. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +0.6% |
| Month | +12.8% |
| Quarter | +0.9% |
| Half Y | +12.7% |
| 1Y | +44.9% |
| YTD | +27.2% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-10-30 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $361.06B | Revenue growth 44.10% y/y |
| Profitability | Gross 29.77%, operating 15.86%, net 9.07% | ROA 5.52%, ROE 12.58% |
| Balance sheet | Cash $10.59B, debt $42.37B | Current ratio 1.14, debt/equity 15.92 |
| Valuation | P/E 25.92, forward P/E 14.58, P/S —, P/B 2.44 | EV/Sales 1.87, EV/EBITDA 9.95 |
| Growth expectations | Earnings growth 112.80%, EPS q/q 105.10% | Analyst mean target $167.00 (22 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 68.76% |
| Insider ownership | 0.08% |
| Short float | 1.15% |
| Short ratio (days to cover) | 2.4 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Blackrock Inc. | 322,741,137 | 7.79% | 2026-03-31 |
| Vanguard Capital Management LLC | 271,198,613 | 6.54% | 2026-03-31 |
| State Street Corporation | 213,904,380 | 5.16% | 2026-03-31 |
| FMR, LLC | 140,010,460 | 3.38% | 2026-03-31 |
| Vanguard Portfolio Management LLC | 115,629,047 | 2.79% | 2026-03-31 |
| Geode Capital Management, LLC | 95,478,649 | 2.30% | 2026-03-31 |
| JPMORGAN CHASE & CO | 85,599,509 | 2.07% | 2026-03-31 |
| Morgan Stanley | 71,718,998 | 1.73% | 2026-03-31 |
| Bank of America Corporation | 68,483,813 | 1.65% | 2026-03-31 |
| Bank Of New York Mellon Corporation | 44,289,134 | 1.07% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $153.96 | EV/Sales | 1.87 |
| Market cap | — | EV/EBITDA | 9.95 |
| Beta | 0.17 | Gross margin | 29.77% |
| RSI(14) | 59.3 | Operating margin | 15.86% |
| ATR(14) | 3.86 | Profit margin | 9.07% |
| SMA20 dist | +2.67% | ROA | 5.52% |
| SMA50 dist | +5.20% | ROE | 12.58% |
| SMA200 dist | +11.18% | Revenue (ttm) | $361.06B |
| 52W high | $170.31 | Revenue growth y/y | 44.10% |
| 52W low | $102.56 | Inst. ownership | 68.76% |
| P/E (ttm) | 25.92 | Insider ownership | 0.08% |
| Forward P/E | 14.58 | Short float | 1.15% |
| PEG (trailing) | 1.32 | Avg volume | 16,737,426 |
| P/S | — | Employees | 57,900 |
| P/B | 2.44 | Analyst rec (1=buy..5=sell) | 2.4 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-08-04 | up | Freedom Broker | Sell → Hold |
| 2026-07-28 | down | B of A Securities | Buy → Neutral |
| 2026-07-09 | main | Mizuho | Neutral → Neutral |
| 2026-07-02 | main | TD Cowen | Buy → Buy |
| 2026-06-29 | main | Morgan Stanley | Overweight → Overweight |
| 2026-06-16 | up | B of A Securities | Neutral → Buy |
| 2026-05-27 | main | Mizuho | Neutral → Neutral |
| 2026-05-26 | main | Barclays | Overweight → Overweight |

## 9. Conclusion
XOM: Moderate momentum / mixed fundamentals / reasonable value / moderate risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for XOM | https://finance.yahoo.com/quote/XOM | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/XOM/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/XOM/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._