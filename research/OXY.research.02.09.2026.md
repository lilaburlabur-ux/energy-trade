# OXY Research Report
**Occidental Petroleum Corporation** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `OXY.research.02.09.2026`

| Field | Value |
|---|---|
| Current price | $60.91 (2026-09-02, ~15-min delayed) |
| Market cap | $60.89B |
| Sector / Industry | Energy / Oil & Gas E&P |
| Main theme | Oil & Gas E&P — see catalysts below |
| Current stance | Moderate technical momentum, strong fundamentals, value: reasonable, risk: moderate. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-09-02.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Moderate | 1Y +30.58%; price +17.43% vs SMA200. |
| Fresh setup quality | Moderate / wait | -7.62% from 52w high; no clean fresh pivot by default. |
| Fundamental quality | Strong | Profit margin 30.32%, revenue growth 53.40%. |
| Value attractiveness | Reasonable | Forward P/E 15.89, EV/Sales 3.36. |
| Risk level | Moderate | Beta 0.16, ATR 2.5% of price, short float 0.01%. |

**Bottom line:** Moderate technical momentum, strong fundamentals, value: reasonable, risk: moderate.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Occidental Petroleum Corporation, together with its subsidiaries, engages in the acquisition, exploration, and development of oil and gas properties in the United States and internationally. It operates through Oil and Gas and Midstream and Marketing. The Oil and Gas segment explores for, develops, and produces oil and condensate, natural gas liquids (NGLs), and natural gas. This segment also optimizes its transportation and storage capacity and invests in entities.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| 1 Value Stock to Own for Decades and 2 Facing Challenges | StockStory | 2026-09-02 |
| Chevron, Exxon and Other Oil Stocks Jump as Two Huge Energy Stories Collide | 24/7 Wall St. | 2026-09-01 |
| The Zacks Analyst Blog Highlights ConocoPhillips, Occidental and National Fuel Gas | Zacks | 2026-09-01 |
| Why ExxonMobil, Chevron, SLB, and Other Energy Stocks Climbed Today | Motley Fool | 2026-09-01 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $60.91; 52w high $65.94 (-7.62%); 52w low $38.56 (+57.97%) |
| Trend | +17.43% vs SMA200, +9.52% vs SMA50, +3.40% vs SMA20 |
| Momentum | RSI(14) 61.6 (neutral) |
| Volatility | ATR(14) 1.50 (~2.5% of price); beta 0.16 |
| Setup perspective | -7.62% from 52w high; no clean fresh pivot by default. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +4.3% |
| Month | +9.8% |
| Quarter | +3.6% |
| Half Y | +14.5% |
| 1Y | +30.6% |
| YTD | +45.1% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-11-09 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $23.93B | Revenue growth 53.40% y/y |
| Profitability | Gross 73.32%, operating 45.44%, net 30.32% | ROA 4.68%, ROE 10.63% |
| Balance sheet | Cash $4.15B, debt $14.63B | Current ratio 1.41, debt/equity 34.51 |
| Valuation | P/E 17.97, forward P/E 15.89, P/S 2.54, P/B 1.82 | EV/Sales 3.36, EV/EBITDA 5.85 |
| Growth expectations | Earnings growth 964.90%, EPS q/q 550.00% | Analyst mean target $66.83 (23 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 57.98% |
| Insider ownership | 26.81% |
| Short float | 0.01% |
| Short ratio (days to cover) | 0.0 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Berkshire Hathaway, Inc | 264,941,431 | 26.50% | 2026-06-30 |
| Dodge & Cox Inc. | 72,764,944 | 7.28% | 2026-06-30 |
| Blackrock Inc. | 55,855,049 | 5.59% | 2026-06-30 |
| Vanguard Capital Management LLC | 48,097,126 | 4.81% | 2026-06-30 |
| State Street Corporation | 46,141,269 | 4.62% | 2026-06-30 |
| Vanguard Portfolio Management LLC | 39,510,431 | 3.95% | 2026-06-30 |
| Geode Capital Management, LLC | 19,900,735 | 1.99% | 2026-06-30 |
| Invesco Ltd. | 12,745,004 | 1.27% | 2026-06-30 |
| Morgan Stanley | 12,240,048 | 1.22% | 2026-06-30 |
| AQR Capital Management, LLC | 11,473,188 | 1.15% | 2026-06-30 |

## 6. Risk Review and Setup Plan
- **Technical trap:** near 52-week highs — do not confuse an old breakout that already worked with a fresh entry.
- **Macro/sector risk:** broad sector sentiment, interest rates, and policy headlines can move the whole group regardless of company-specific execution.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $60.91 | EV/Sales | 3.36 |
| Market cap | $60.89B | EV/EBITDA | 5.85 |
| Beta | 0.16 | Gross margin | 73.32% |
| RSI(14) | 61.6 | Operating margin | 45.44% |
| ATR(14) | 1.50 | Profit margin | 30.32% |
| SMA20 dist | +3.40% | ROA | 4.68% |
| SMA50 dist | +9.52% | ROE | 10.63% |
| SMA200 dist | +17.43% | Revenue (ttm) | $23.93B |
| 52W high | $65.94 | Revenue growth y/y | 53.40% |
| 52W low | $38.56 | Inst. ownership | 57.98% |
| P/E (ttm) | 17.97 | Insider ownership | 26.81% |
| Forward P/E | 15.89 | Short float | 0.01% |
| PEG (trailing) | 1.24 | Avg volume | 8,868,460 |
| P/S | 2.54 | Employees | 10,412 |
| P/B | 1.82 | Analyst rec (1=buy..5=sell) | 2.3 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-08-17 | main | Barclays | Overweight → Overweight |
| 2026-08-11 | main | Susquehanna | Positive → Positive |
| 2026-08-10 | main | Truist Securities | Hold → Hold |
| 2026-08-07 | main | Wells Fargo | Overweight → Overweight |
| 2026-08-06 | main | Barclays | Overweight → Overweight |
| 2026-07-17 | main | Citigroup | Neutral → Neutral |
| 2026-07-14 | main | Stephens & Co. | Overweight → Overweight |
| 2026-07-08 | up | Evercore ISI Group | Underperform → Outperform |

## 9. Conclusion
OXY: Moderate momentum / strong fundamentals / reasonable value / moderate risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for OXY | https://finance.yahoo.com/quote/OXY | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/OXY/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/OXY/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._